...
class PokedexUI:
    ...
    def __init__(self, screen, game):
        ...
        self.calepin = pygame.image.load("imgs/calepin.png")

        # Fonts (fallback to default if missing)
        try:
            self.font_title = pygame.font.Font("fonts/Pokedex.ttf", 22)
            self.font_text = pygame.font.Font("fonts/Pokedex.ttf", 18)
        except Exception:
        ...
        self.box_rect = self.screen.blit(self.calepin, (config.SCREEN_WIDTH // 3, config.SCREEN_HEIGHT // 3.5))
    ...
    def render(self):
        ...
        # Main box
        self.screen.blit(self.calepin, (config.SCREEN_WIDTH // 3, config.SCREEN_HEIGHT // 3.5))     #Image de fond
        ...
        # Title + page
        title = f"Carnet de Charlie — Monde {self.world_page}"
        title_surf = self.font_title.render(title, True, (0, 0, 0))
        self.screen.blit(title_surf, (self.box_rect.x + 30, self.box_rect.y + 35))

        hint = "P = fermer ; flèches pour dérouler"
        hint_surf = self.font_text.render(hint, True, (0, 0, 0))
        self.screen.blit(hint_surf, (self.box_rect.right - hint_surf.get_width() - 20, self.box_rect.y + 35))
        ...
        per_page = max(1, area_h // (self.entry_h + self.entry_gap)) # // (self.entry_h + self.entry_gap)
