...
class PNJ9:
    def __init__(self, screen, game, player):
        ...
        self.choices = ["0"]
        self.choices1 = ["Vous n'auriez pas une clef, par hasard ?", "C'est vous Lux ?"]
        self.choices1status = ["attente"]
        self.choices2 = ["Vous êtes Cordelia ?", "Vous n'auriez pas une clef, par hasard ?"]
    ...
    def render(self):
        ...
        if self.game.Maison_papier_dialogue_done == True and self.game.PNJ10_dialogue_done == False and self.cut < 13:
            self.cut = 13
            return

        if self.game.PNJ10_dialogue_done == True and self.game.PNJ7_2_dialogue_done == False and self.cut < 15:
            self.cut = 15
            return

        if self.game.PNJ7_2_dialogue_done == True and self.cut < 17:
            self.cut = 17
            return
    ...
    def handle_choice(self):
        ...
        if choices == "Vous n'auriez pas une clef, par hasard ?" :
            self.choice_active = False
            self.goodbye = True
            self.current_char = 0
            self.last_update = pygame.time.get_ticks()

        elif choices == "C'est vous Lux ?":
            self.choice_active = False
            self.choices1status = ["fini"]
            self.current_char = 0
            self.last_update = pygame.time.get_ticks()

        elif choices == "Vous êtes Cordelia ?":
            self.choice_active = False
    ...
    def render_goodbye(self):
        self._render_dialogue_box("Une clef ? Même si je l'avais, je la donnerais pas à n'importe qui pardi !")
        self.screen.blit(self.charlie_penaud_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

#SCENE ZERO
    def render_scene_0(self):
        self._render_dialogue_box("Si je suis perdux dans le mauvais espace-temps ? Ahahah, mais bien sûr ! Et demain, on se fait envahir par une espèce intergalactique ? Ahahah !")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
        self.game.unlock_pokedex_entry(2, "PNJ9.0")
...
#SCENE UN
    def render_scene_2(self):
        self._render_dialogue_box("Ah non, moi c'est Cordelia. Lux est souvent dehors à cette heure-ci.")
        self.screen.blit(self.charlie_penaud_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
...
#SCENE DEUX
    def render_scene_4(self):
        self._render_dialogue_box("Oui c'est moi Cordelia. Vous voulez quelque chose ?")
        self.screen.blit(self.charlie_penaud_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_5(self):
        self._render_dialogue_box_Answer("Enfin ! Je vous cherchais partout. J’ai croisé Lux qui m’a dit que vous aviez la clef du labo. Vous l’avez encore ?")
        self.screen.blit(self.charlie, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_6(self):
        self._render_dialogue_box("Ah non, non je l’ai plus, je l’ai passée à mu biauz froeur, Priss, comme al me l’avait demandé.")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_7(self):
        self._render_dialogue_box_Answer("Evidemment, c’aurait été trop beau... Vous savez où je peux trouver Priss ?")
        self.screen.blit(self.charlie_surpris, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_8(self):
        self._render_dialogue_box("Ah ben vous lu trouverez pas, al est partix en tournée. C’est an granx comédian vous savez ! Mais al a sans doute laissé la clef chez al, vous pouvez aller la chercher.")
        self.screen.blit(self.charlie_surpris_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
        self.game.unlock_pokedex_entry(2, "PNJ9.1")

    def render_scene_9(self):
        self._render_dialogue_box("Par contre je dois vous prévenir, al est assez bordélique...")
        self.screen.blit(self.charlie_surpris_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_10(self):
        self._render_dialogue_box("")
        self.game.PNJ9_dialogue_done = True

#SCENE TROIS
    def render_scene_11(self):
        self._render_dialogue_box("Allez chercher dans la maison de Priss, y a sans doute la clef.")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
...
#SCENE QUATRE
    def render_scene_13(self):
        self._render_dialogue_box("Un mot ? Trouvé chez Priss ? Aucune idée, c'est pas moi en tout cas. On dirait une écriture d'enfant.")
        self.screen.blit(self.charlie_penaud_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
...
#SCENE SIX
    def render_scene_17(self):
        self._render_dialogue_box("Vous avez votre clef ?")
        self.screen.blit(self.charlie, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))