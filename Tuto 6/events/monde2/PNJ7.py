...
class PNJ7:
    def __init__(self, screen, game, player):
        ...
        self.choices = ["0"]
        self.choices1 = ["Tu te serais pas fichu de moi ?", "Camille !!!"]
    ...
    def render(self):
        ...
        if self.game.PNJ8_dialogue_done == False and self.game.PNJ7_dialogue_done == True and self.cut < 7:
            self.cut = 7
            return
        ...
        if self.game.PNJ9_dialogue_done == True and self.game.Maison_papier_dialogue_done == False and self.cut < 13:
            self.cut = 13
            return

        if self.game.Maison_papier_dialogue_done == True and self.game.PNJ10_dialogue_done == False and self.cut < 20:
            self.cut = 20
            return
        ...
        if self.game.PNJ7_2_dialogue_done == True and self.cut < 35:
            self.cut = 35
            return
    ...
    def handle_choice(self):
        ...
        if choices == "Tu te serais pas fichu de moi ?" :
            self.choice_active = False
            self.choices1status = ["fini"]
            self.current_char = 0
            self.last_update = pygame.time.get_ticks()

        elif choices == "Camille !!!":
            self.choice_active = False
            self.choices1status = ["fini"]
            self.current_char = 0
            self.last_update = pygame.time.get_ticks()
...
#SCENE ZERO
    def render_scene_0(self):
        self._render_dialogue_box_Answer("Excuse-moi , tu sais comment je peux faire pour rentrer à la Scène Errès ?")
        self.screen.blit(self.charlie, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_1(self):
        self._render_dialogue_box("Oh, oui, c’est lu portaer qui garde les clefs normalement. Mais al est malade, alors c'est nous qui les gardons, on se les fait passer quand on en a besoin.")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_2(self):
        self._render_dialogue_box_Answer("« On » ? C’est qui « on » ?")
        self.screen.blit(self.charlie_surpris, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_3(self):
        self._render_dialogue_box("Euh je sais pas, moi je les ait passées à Lux, qui les a passées à Cordelia, qui les a refilées à Anatol je crois. Enfin de ce que je sais ! Enfin en tout cas je suis certan de les avoir passées à Lux au début.")
        self.screen.blit(self.charlie_surpris_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_4(self):
        self._render_dialogue_box_Answer("Et je peux le trouver où ce Lux ?")
        self.screen.blit(self.charlie_surpris, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_5(self):
        self._render_dialogue_box("Al traine souvent près de la rivière au niveau de la Scène Errès. Dis-lui que c'est Camille qui t'envoie.")
        self.screen.blit(self.charlie_surpris_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
        self.game.unlock_pokedex_entry(2, "PNJ7.0")

    def render_scene_6(self):
        self._render_dialogue_box("")
        self.game.PNJ7_dialogue_done = True

#SCENE 1
    def render_scene_7(self):
        self._render_dialogue_box("Vas voir Lux, al a peut-être encore les clefs. Al est sans doute à jouer au Nord, près de la rivière.")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
...
#SCENE 2
    def render_scene_9(self):
        self._render_dialogue_box("Encore toi ? T'as toujours pas trouvé les clefs ? Eh bah... Pas jojo, ta journée...")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_10(self):
        self._render_dialogue_box_Answer("Ouais... Dis tu voudrais pas m'aider ? Je cherche Cordelia du coup.")
        self.screen.blit(self.charlie, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_11(self):
        self._render_dialogue_box("Ah ben non je peux pas t'aider, je joue moi ! Mais enfin, j'pense qu'al doit trainer chez al.")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
...
#SCENE TROIS
    def render_scene_13(self):
        self._render_dialogue_box("Ahahahah tu traines encore ? En fait c'est bien plus drôle de te voir galérer que de jouer !")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_14(self):
        self._render_dialogue_box_Answer("Ahah, merci de ton soutien... J'imagine que tu ne vas pas m'aider à trouver la maison de Priss ?")
        self.screen.blit(self.charlie, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_15(self):
        self._render_dialogue_box("Bah, j'pourrais te le dire, mais t'as la clef pour rentrer dans sa maison ?")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_16(self):
        self._render_dialogue_box_Answer("De QUOI ?! Une autre clef à chercher ?!?")
        self.screen.blit(self.charlie_surpris, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_17(self):
        self._render_dialogue_box("Nan, j'rigole ! Al ferme jamais sa porte, t'as qu'à pousser ça va s'ouvrir tout seul.")
        self.screen.blit(self.charlie_surpris_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_18(self):
        self._render_dialogue_box_Answer("Super... Je m'en réjouirais presque...")
...
#SCENE QUATRE
    def render_scene_20(self):
        self._render_dialogue_box("C'est pas mon écriture, j'ai appris à écrire en lettres cursives, moi !")
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
...
#SCENE CINQ 
    def render_scene_22(self):
        self._render_dialogue_box_Answer("On m’a dit que c’était toi qui avait la clef ! C’est vrai cette histoire ? ")
        self.screen.blit(self.charlie_mécontent, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_23(self):
        self._render_dialogue_box("Mais qu'est-ce qui te fait dire ça ?")
        self.screen.blit(self.charlie_mécontent_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_24(self):
        self._render_dialogue_box_Answer("La ville entière ! J'ai dû aller voir Lux, qui l'avait donné à Cordelia, qui l'a refilé à Priss parti en tournée, oui oui je sais il joue très bien un grand comédien et tout, et là je trouve un mot appartenant à Anatol qui me dit que c'est toi qui a la clef !")
        self.screen.blit(self.charlie_mécontent, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
    ...
    def render_scene_26(self):
        self._render_dialogue_box("Oui c'est moi qui l'avais, ahahahah c'était bien marrant de te voir galérer comme ça ! C'était dur de ne pas rire devant toi !")
        self.screen.blit(self.charlie_mécontent_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_27(self):
        self._render_dialogue_box_Answer("Ouais ben merci, grâce à toi je connais tout le monde ici... Bon tu me la passe cette clef ?")
        self.screen.blit(self.charlie_mécontent, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_28(self):
        self._render_dialogue_box("Oui, ahah c'est bon j'ai bien ri, la voilà ta clef !")
        self.screen.blit(self.charlie_mécontent_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_29(self):
        self._render_dialogue_box("..............................Je la retrouve plus...")
        self.screen.blit(self.charlie_mécontent_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_30(self):
        self._render_dialogue_box_Answer("Tu QUOI ??!!")
        self.screen.blit(self.charlie_mécontent, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_31(self):
        self._render_dialogue_box("Elle a dû tomber de ma poche à un moment... Oupsi...")
        self.screen.blit(self.charlie_mécontent_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_32(self):
        self._render_dialogue_box_Answer("C’est une farce j’espère !")
        self.screen.blit(self.charlie_mécontent, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_33(self):
        self._render_dialogue_box("Mais elle doit pas être bien loin ! J’ai dû la faire tomber près des toilettes, t’as qu’à chercher par là-bas... Bon ben je te laisse chercher hein ! ")
        self.screen.blit(self.charlie_mécontent_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))

    def render_scene_34(self):
        self._render_dialogue_box("")
        self.game.PNJ7_2_dialogue_done = True
        
#SCENE SIX
    def render_scene_35(self):
        self._render_dialogue_box("Allez, fais pas la tête... C'était marrant quoi ! J'ai dû perdre la clef vers les toilettes.")
        self.screen.blit(self.charlie_mécontent_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))