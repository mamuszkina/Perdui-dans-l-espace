...
class tag_maison:
    ...
    def render(self):
        if self.game.go_gandalf == False and self.game.Gandalf5_dialogue_done == False and self.cut == 0:
            self.cut = 6

        if self.game.go_gandalf == True and self.game.Gandalf5_dialogue_done == False and self.cut == 0:
            self.cut = 6
    ...
    def render_scene_0(self):
        self._render_dialogue_box_Answer("Gandalf, tu vois ce tag ? Tu pourrais traduire ce qui écrit dessus ?")
        
    def render_scene_1(self):
        self._render_dialogue_box_Gandalf("Je peux toujours essayer oui. Je lis... C'est une citation. Elle dit 'Quand le è-serpent tire la è-langue, le à-pigeon s'enfuit.'")
        
    def render_scene_2(self):
        self._render_dialogue_box_Answer(" è-serpent ? è-langue ? à-pigeon ? Tu es sûr de toi ?")
        
    def render_scene_3(self):
        self._render_dialogue_box_Gandalf("Les è- et à- ne se traduisent pas dans ta langue. Mais ils sont bien présents.")
        self.game.compris = True
        
    def render_scene_4(self):
        self._render_dialogue_box_Answer("On a de nouveaux indices alors. Allons les noter sur le panneau d'affichage.")
        self.game.serpent = True
        self.game.pigeon = True
        self.game.langue = True

    def render_scene_5(self):
        self._render_dialogue_box_Answer("")

    def render_scene_6(self):
        self._render_dialogue_box_Answer_no_Gandalf("Oh, il y a un tag sur cette maison. On dirait une écriture... Bon, évidemment, je ne comprend rien. Je ferais mieux d'aller voir Gandalf.")
