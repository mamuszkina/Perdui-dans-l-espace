...
class panneau17:
    ...
    def _render_dialogue_box_Answer_no_Gandalf_mécontent(self, text):
        dialogue_rect = self.screen.blit(self.dialog, (config.SCREEN_WIDTH // 2.8, config.SCREEN_HEIGHT // 1.4))      # chiffres = position bulle de dialogue
    ...
    def render_scene_0(self):
        self._render_dialogue_box_PNJ("URGENT CHERCHE è-CORDE POUR CONCERT")

    def render_scene_1(self):
        self._render_dialogue_box_static("URGENT CHERCHE è-CORDE POUR CONCERT")
        self._render_dialogue_box_Answer("Gandalf, ça dit quoi ?")

    def render_scene_2(self):
        self._render_dialogue_box_static("URGENT CHERCHE è-CORDE POUR CONCERT")
        self._render_dialogue_box_Gandalf("C'est écrit 'urgent cherche è-corde pour concert.")
        
    def render_scene_3(self):
        self._render_dialogue_box_static("URGENT CHERCHE è-CORDE POUR CONCERT")
        self._render_dialogue_box_Answer("è-corde ? Avec un 'è-' devant ? Tu es sûr de toi ? ")
        
    def render_scene_4(self):
        self._render_dialogue_box_static("URGENT CHERCHE è-CORDE POUR CONCERT")
        self._render_dialogue_box_Gandalf("Le è- ne se traduit pas dans ta langue. Mais il y a bien un è- devant le mot corde.")
        self.game.compris = True
        
    def render_scene_5(self):
        self._render_dialogue_box_static("URGENT CHERCHE è-CORDE POUR CONCERT")
        self._render_dialogue_box_Answer("On met un 'è-' devant corde. Allons noter ça.")
        self.game.corde = True
    ...
    def render_scene_7(self):
        self._render_dialogue_box_static("URGENT CHERCHE è-CORDE POUR CONCERT")
        self._render_dialogue_box_Answer_no_Gandalf_mécontent("Hein ? Evidemment, je comprend strictement rien. Super. Bon, j'ai plus qu'à aller voir Gandalf. Comme d'habitude...")
