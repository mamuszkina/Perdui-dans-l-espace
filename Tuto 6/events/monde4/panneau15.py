...
#SCENE ZERO 
    def render_scene_0(self):
        self._render_dialogue_box("SCENE ERRES RECHERCHE UN EMPLOYE POUR VEILLER SUR SA SEIGNEURIE")

    def render_scene_1(self):
        self._render_dialogue_box_static("SCENE ERRES RECHERCHE UN EMPLOYE POUR VEILLER SUR SA SEIGNEURIE")
        self._render_dialogue_box_Answer("Ah, ça fait du bien, des choses écrites en Français. On est clairement pas à Paris, je reconnais rien, mais on est peut-être en France ? Je serais arrivé chez moi ?")
        self.screen.blit(self.charlie, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
        
    def render_scene_2(self):
        self._render_dialogue_box_static("SCENE ERRES RECHERCHE UN EMPLOYE POUR VEILLER SUR SA SEIGNEURIE")
        self._render_dialogue_box_Answer("Pas sûr, avec mon poisson traducteur on sait jamais... Je ferais mieux d'aller parler à des gens, sinon je suis bon pour chercher Gandalf... Encore une fois.")
