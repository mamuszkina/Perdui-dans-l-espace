...
class PNJ15:
    ...
    def _render_dialogue_box2(self, text):
        dialogue_rect = self.screen.blit(self.dialogPNJ, (config.SCREEN_WIDTH // 2.8, config.SCREEN_HEIGHT // 1.4))      # chiffres = position bulle de dialogue
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
    ...
    def _render_dialogue_box3(self, text):
        dialogue_rect = self.screen.blit(self.dialogPNJ, (config.SCREEN_WIDTH // 2.8, config.SCREEN_HEIGHT // 1.4))      # chiffres = position bulle de dialogue
        self.screen.blit(self.charlie_gray, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
        self.screen.blit(self.PNJ3, (config.SCREEN_WIDTH // 6, config.SCREEN_HEIGHT * 0.5))
    ...
    def render_scene_0(self):
        self._render_dialogue_box("Ahah !")

    def render_scene_1(self):
        self._render_dialogue_box2("Hoho !")
        
    def render_scene_2(self):
        self._render_dialogue_box3("Magda ! Ne jette pas de poudre INUN aux yeux des autres enfants !")
        self.game.unlock_pokedex_entry(4, "PNJ17")

    def render_scene_3(self):
        self._render_dialogue_box_Answer("Hein ? C'était quoi ça ?")