...
class Gandalf3:
    def __init__(self, screen, game, player):
        ...
        self.lightning_sound = pygame.mixer.Sound("lightning.ogg")
    ...
    def update(self):
        ...
        if self.cut > self.max_cut:
            self.game.teleport_to_map("monde4", [8, 29])  # [1, 4] = entry position on map 01
    ...
    def render(self):
        if self.game.Gandalf3_QuiEtesVous_dialogue_done == True and self.game.Gandalf3_KABOOM_dialogue_done == False and self.game.Gandalf3_UniversAvant_dialogue_done == True and self.cut == 11 or self.cut == 24 or self.cut == 33 :
    ...
    def render_scene_17(self):
        self._render_dialogue_box("C'est le destin de toutes les langues d'être 'dénaturées'. Les langues changent en fonction de leur utilisation, c'est ce qui fait qu'une langue est en vie. Tu savais que dans certains univers, des institutions ont décrété il y a des centaines d'années de ça que 'le genre masculin est le plus pur' et que c'est à cause de ça que le masculin générique est utilisé, et que ça n'a rien de naturel ?")
    ...
    def render_scene_68(self):
        self.render_dialogue_box_Lightning("KABOOM")