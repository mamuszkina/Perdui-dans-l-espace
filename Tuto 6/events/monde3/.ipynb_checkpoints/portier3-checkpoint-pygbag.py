...
class portier3:
    def __init__(self, screen, game, player):
        ...
        self.monster_pad = pygame.transform.scale(self.monster_pad, (config.SCREEN_WIDTH//2, config.SCREEN_HEIGHT//2))
        self.name_card = pygame.image.load("imgs/battle/name_card.png").convert_alpha()
        self.name_card = pygame.transform.scale(self.name_card, (config.SCREEN_WIDTH//2, config.SCREEN_HEIGHT//2))
        self.hp_bar = pygame.image.load("imgs/battle/hp_bar.png").convert_alpha()
        self.hp_bar = pygame.transform.scale(self.hp_bar, (config.SCREEN_WIDTH//2, config.SCREEN_HEIGHT//2))
        self.menu = pygame.image.load("imgs/battle/menu.png").convert_alpha()
        self.menu = pygame.transform.scale(self.menu, (config.SCREEN_WIDTH, config.SCREEN_HEIGHT//3))
        self.menu2 = pygame.image.load("imgs/battle/menu2.png").convert_alpha()
        self.menu2 = pygame.transform.scale(self.menu2, (config.SCREEN_WIDTH//3, config.SCREEN_HEIGHT//3))
        self.portier3 = pygame.image.load("imgs/battle/prof3.png").convert_alpha()
        self.portier3 = pygame.transform.scale(self.portier3, (config.SCREEN_WIDTH//7, config.SCREEN_HEIGHT//6))
        ...
        #son combat pokémon
        self.pokemon_sound = pygame.mixer.Sound("sons/pokemonfight.ogg")
...
#        dialogue_rect = self.screen.blit(self.menu,(config.SCREEN_WIDTH // 6, config.SCREEN_HEIGHT // 1.5))
        ...
        #Charlie
        self.screen.blit(self.monster_pad, (config.SCREEN_WIDTH // 6, config.SCREEN_HEIGHT // 1.5))
        self.screen.blit(self.name_card, (config.SCREEN_WIDTH // 2.8, config.SCREEN_HEIGHT // 1.3))
        self.screen.blit(self.hp_bar, (config.SCREEN_WIDTH // 2.8, config.SCREEN_HEIGHT // 1.3))
        self.screen.blit(self.Charlie, (config.SCREEN_WIDTH // 5.2, config.SCREEN_HEIGHT // 1.7))
        self.screen.blit(font.render("Charlie", True, config.BLACK), (config.SCREEN_WIDTH // 2.8, config.SCREEN_HEIGHT // 1.3))

        #Portier3
        self.screen.blit(self.monster_pad, (config.SCREEN_WIDTH // 8, config.SCREEN_HEIGHT // 1.))
        self.screen.blit(self.name_card, (config.SCREEN_WIDTH // 4.8, config.SCREEN_HEIGHT // 0.8))
        self.screen.blit(self.hp_bar, (config.SCREEN_WIDTH // 4.8, config.SCREEN_HEIGHT // 1))
        self.screen.blit(self.portier3, (config.SCREEN_WIDTH//1.7, config.SCREEN_HEIGHT//5.2))
        self.screen.blit(font.render("Portier3", True, config.BLACK), (80, 111))

        monster_percent = self.health / self.base_health
        monster_color = self.determine_health_color(monster_percent)
        pygame.draw.rect(self.screen, monster_color, pygame.Rect(141, 142, 202 * monster_percent, 16))
        ...
        pygame.draw.rect(self.screen, player_monster_color, pygame.Rect(596, 392, 202 * player_monster_percent, 16))
 ...
 #           self.handle_choice1
            ...
            self.active_phrase_text = "Ce soir, le rôle d'Ali sera joué par eune grante ténébreusse, sinistrë et moche !"
    ...
    def _render_dialogue_box_Battle(self, text):
        dialogue_rect = self.screen.blit(self.menu, (0, config.SCREEN_HEIGHT // 1.35))      # chiffres = position bulle de dialogue
        ...
        for line in wrapped:
            ...
            visible = line[:chars_left]
            surf = font.render(visible, True, color)
            self.screen.blit(surf, (dialogue_rect.x + 45, y))         # chiffre, y = décalage début de bulle
            chars_left -= len(line) + 1
            y += font.get_height() + 6 
    ...
    def _render_dialogue_box_Battle_Phrase(self, text):
        ...
        rect = self.menu.get_rect(center=(config.SCREEN_WIDTH // 3, config.SCREEN_HEIGHT // 7))
        dialogue_rect = self.screen.blit(self.menu, rect.topleft)
    ...
    def render_choice(self):
        if (44 <= self.cut <= 79):
            ...
            dialogue_rect = self.screen.blit(self.menu, (0, 485))
        else :
            dialogue_rect = self.screen.blit(self.dialog, (config.SCREEN_WIDTH // 2.8, config.SCREEN_HEIGHT // 1.4))      # chiffres = position bulle de dialogue
        self.screen.blit(self.charlie, (config.SCREEN_WIDTH // 1.6, config.SCREEN_HEIGHT * 0.5))
        self.screen.blit(self.PNJ_gray, (config.SCREEN_WIDTH // 6, config.SCREEN_HEIGHT * 0.5))
...
#SCENE QUATRE
    ...
    def render_scene_37(self):
        self._render_dialogue_box_Answer("Allez, vas essayer et reviens me raconter !")
...
#SCENE SIX
    ...
    def render_scene_44(self):
        ...
        if not self.pokemon_played:
            ...
            self.pokemon_sound.play()         # Joue le tonnerre
    ...
    def render_scene_79(self):
        self._render_dialogue_box("Tiens, tiens, tiens, des visiteurzes... Transgresseurx ! Espiomes ! Pas des espiomes !")
    ...
    def render_scene_80(self):
        if self.pokemon_played:
            ...
            self.pokemon_played = False
        self._render_dialogue_box("Ouah tu es vraiment devenu un pro ! Bravo à toi, tu peux aller voir Gandalf, encore bravo hein !")
